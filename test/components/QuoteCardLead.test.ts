...
describe('quoteCard', () => {
  ...
  const render = async ({ className }: RenderOptions = {}) => {
    ...
    return await container.renderToString(QuoteCardLead, {
      props: {
        quote: quote,
        class: className,
      },
  ...
  test('that the card uses the chalk font', async () => {
  ...
  test('that the card contains source title and type when provided', async () => {
    ...
    const container = await AstroContainer.create()
    const html = await container.renderToString(QuoteCardLead, {
